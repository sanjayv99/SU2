...
void CRadialBasisFunctionInterpolation::SolveRBF_System(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const su2double radius, bool Derivative, vector<unsigned long>& internalNodes, bool ForwardProjectionDerivative){
  ...
  if(config->GetRBF_DataReduction()){
    ...
    while(((MaxErrorGlobal > dataReductionTolerance)  || greedyIter == 0)){ 
      ...
      for (const auto &itype : ctrltypes) {
        ...
        for (auto mark : marker){
          ...
          if (std::find(CtrlTypeVec.begin(), CtrlTypeVec.end(), itype) == CtrlTypeVec.end()) { // TODO -  not very elegant
    ...
    for (auto mark : markers) {
      ...
      auto ADT = CreateADT_per(geometry, "edge", mark.first);
      auto indicesSet = mark.second;
      ProjectBoundNodes(geometry, config, type, radius, "edge", ADT.get(), true, per_nodes, &indicesSet);
    }
    ...
    */
    ...
    for (auto mark : markers) {
      ...
      auto ADT = CreateADT_per(geometry, "surface", mark.first);
      auto indicesSet = mark.second;
    ...
    // BoundADT = CreateADT(geometry, "surface");
    ...
    for (auto iType : types) {
      if (node_indices[iType].size() > 0) {
        ...
        for (const auto& iMarker : markers) {
          ...
          // auto BoundADT = CreateADT(geometry, iType, iMarker.first);
        ...
        // auto sliding_idx = GetIndices(node_indices, iType); // TODO -  change var name
              ...
              for (auto jMarker = 0u; jMarker < config->GetnMarker_All(); jMarker++){
                ...
                if(geometry->nodes->GetVertex(iNode, jMarker) != -1 && config->GetMarker_All_PerBound(jMarker)){
                  ...
                  if(idx_PeriodicMarker < config->GetMarker_Periodic_Donor2(idx_PeriodicMarker)){
                    nodes.push_back(new CRadialBasisFunctionNode(iNode, iMarker, iVertex));
                    nodes.back()->setNodetype("edge"); 
                    nodes.back()->SetDomainVertex(); // TODO -  Function can be removed
                  }else{
                    per_nodes.push_back(new CRadialBasisFunctionNode(iNode, iMarker, iVertex));
    ...
    for (auto mark : test){
      for (auto idx_i : mark.second) {
        ...
        if (((config->GetMarker_All_Moving(iMarker) == YES) && (Kind_SU2 == SU2_COMPONENT::SU2_CFD)) ||
          ...
          GetNodalDeformation(geometry, nodes[idx_i] ,varCoord);// TODO -  
...
void CRadialBasisFunctionInterpolation::UpdateBoundCoords(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const su2double radius){
  ...
  if(config->GetRBF_DataReduction()){
    vector<string> types = {"displaced","edge", "surface"}; // TODO -  
    for (auto nodetype : types) {
      /*--- Looping over the non selected boundary nodes ---*/
      // auto idx = GetIndices(node_indices, nodetype);
...
void CRadialBasisFunctionInterpolation::SetCtrlNodeCoords(CGeometry* geometry, CConfig* config){
  ...
  unsigned long idx = 0;
  for (auto type : CtrlTypeVec) {
    // auto ctrl_idx = config->GetRBF_DataReduction() ? GetIndices(ctrl_nodes_type, type) : GetIndices(node_indices, type);// TODO -  find replacement
    // auto indices = config->GetRBF_DataReduction() ? GetIndices(ctrl_node_indices,type) : GetIndices(node_indices,type);
...
void CRadialBasisFunctionInterpolation::GetInterpError(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const su2double radius, bool Derivative, vector<string>& ctrltypes,  unsigned long& maxErrorNodeLocal, su2double& maxErrorLocal){
  ...
  for (auto i_type : ctrltypes) {
    ...
    for (auto mark : nodes_edge) {
      auto BoundADT = CreateADT(geometry, i_type, mark.first);

      auto indices = mark.second;  

      ProjectBoundNodes(geometry, config, type, radius, i_type, BoundADT.get(), true, nodes, &indices);
    ...
    // auto project_idx = GetIndices(node_indices, i_type); // TODO -   change var name
    
    // TODO -   fix
    // indices = node_indices[i_type];
...
void CRadialBasisFunctionInterpolation::SetCorrection(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const vector<unsigned long>& internalNodes){
  ...
  // TODO -  add note that selected control nodes are also included as to avoid problems with high aspect ratio cells near boundaries!
...
unique_ptr<CADTPointsOnlyClass> CRadialBasisFunctionInterpolation::CreateADT(CGeometry* geometry, const string& type, const unsigned short marker){
  // TODO -   the periodic surface nodes should be included. Likely the previous implementation where they have an "edge" nodetype would then also work. But those edges have to be included here. 
  auto indices = node_indices[type][marker];
  auto per_indices = per_node_indices[type][marker];

  unsigned long size = indices.size() + per_indices.size();
  ...
  for (auto idx_i : indices){
    ...
    PointIDs[cnt] = nodes[idx_i]->GetVertex(); 
  ...
  for (auto idx_i : per_indices) {
    if(per_nodes[idx_i]->GetDomainVertex() || geometry->nodes->GetPeriodicBoundary(per_nodes[idx_i]->GetIndex())){
      domainVertexCnt++;
      continue;
    }
    PointIDs[cnt] = per_nodes[idx_i]->GetVertex(); 
    su2double* coord = IsCylindrical ? per_nodes[idx_i]->GetCylCoord() : geometry->nodes->GetCoord(per_nodes[idx_i]->GetIndex());
    for (auto iDim = 0u; iDim < nDim; iDim++){
      Coord_bound[cnt*nDim + iDim] = coord[iDim];
    }
    cnt++;
...
void CRadialBasisFunctionInterpolation::ProjectBoundNodes(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const su2double radius, const string& nodetype, CADTPointsOnlyClass* BoundADT, bool SetError, vector<CRadialBasisFunctionNode*>& target_nodes, vector<unsigned long>* project_idx) {
  ...
  for (auto x : indices) {
    ...
    // TODO -  start here check if works correctly for some test cases and then check if projection functions can be made more concise
    bool isEdge3D = (nDim == 3 && nodetype == "edge");    
  
    ApplyProjection(geometry, config, iMarker, pointID, isEdge3D, target_nodes[x]->getNodetype(), coord, new_coord, projection);  // This should return the projection  
    ...
    if (SetError) {
      ...
      // if(nodetype == "surface" && geometry->nodes->GetPeriodicBoundary(node)){
      //   cout << "error mag: " << GeometryToolbox::SquaredNorm(nDim, projection) << endl;
      // 
...
void CRadialBasisFunctionInterpolation::ApplyProjection(CGeometry* geometry, CConfig* config, unsigned short iMarker, unsigned long pointID, bool isEdge3D, const string& nodetype, su2double* coord, su2double* new_coord, su2double* projection) {
  // check the diff between these two and might be able to combine them in a smart way
  ProjectDefault(geometry, config, iMarker, pointID, new_coord, nodetype, projection);
}


void CRadialBasisFunctionInterpolation::ProjectDefault(CGeometry* geometry, CConfig* config, unsigned short iMarker, unsigned long pointID, su2double* new_coord, const string& nodetype, su2double* projection) {

  // START HERE something funky is going on with the normal when transforming to cylindrical coords in case of a vertex node
  ...
  // cout << "unit normal: " << normal[0]/mag_normal << " " << normal[1]/mag_normal << " " << normal[2]/mag_normal << endl;
  ...
  if (nodetype=="edge" && nDim == 3){
    ...
    // TODO -  this if statement can likely be removed
    if (mark == config->GetnMarker_All()) {
      SU2_MPI::Error("ProjectEdge3D has only found a single marker for a 3D edge!!!!", CURRENT_FUNCTION);
    }
    ...
    if (IsCylindrical){
      delta_Cart_to_cyl(geometry->vertex[iMarker][pointID]->GetCoord(), cart_normal, normal2);
    } else{
