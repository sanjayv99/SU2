...
void CRadialBasisFunctionInterpolation::SetVolume_Deformation(CGeometry* geometry, CConfig* config, bool UpdateGeo, bool Derivative,
  ...
  for (auto iNonlinear_Iter = 0ul; iNonlinear_Iter < 1; iNonlinear_Iter++) {
    ...
    if (rank == MASTER_NODE && Screen_Output) {
      cout << "Non-linear iter.: " << iNonlinear_Iter + 1 << "/" << nIter << ". ";
      if (nDim == 2)
...
void CRadialBasisFunctionInterpolation::SolveRBF_System_IL(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const bool Derivative, const bool ForwardProjectionDerivative, const bool Screen_Output) {
    ...
    for (auto iMarker : node_indices[NODETYPE::IL_WALL]){        
      ...
      if (dataReductionCfg) {
        ...
        for (auto iNode : iMarker.second) {
          ...
          su2double def = GeometryToolbox::SquaredNorm(nDim, varCoord);
          if (def > maxDef) {
            maxDef = def;
            maxNode = iNode;
          }
        ...
        nodes[maxNode]->setControl();

        vector<su2double> zero(nDim, 0.0);
        nodes[maxNode]->SetError(zero.data(), nDim);

        auto marker = nodes[maxNode]->GetMarker();
        if (find(secondaryMarker.begin(), secondaryMarker.end(), marker) != secondaryMarker.end()) {
          marker=sec2prim[marker];
        }
        auto tag = config->GetMarker_All_TagBound(marker);
        auto global_marker = config->GetMarker_CfgFile_TagBound(tag);
        ctrl_node_indices[NODETYPE::IL_WALL][global_marker].insert(maxNode);
        ...
        while(sqrt(maxDef) > tol) {
          ...
          for (auto iNode : iMarker.second) {          
            ...
            def << nodes[iNode]->GetNewCoord()[0] << "\t" << nodes[iNode]->GetNewCoord()[1] << endl;
            // GetNodalError(geometry, config, type, radius_IL, nodes[iNode], Derivative, localError);
            ...
            for (auto iDim = 0u; iDim<nDim;iDim++) {
              delta[iDim] = nc[iDim] - c[iDim];
              localError[iDim] = -vc[iDim] + delta[iDim];
            }

            nodes[iNode]->SetError(localError, nDim);
            err << localError[0] << "\t" << localError[1] << endl;
            su2double errMag = GeometryToolbox::SquaredNorm(nDim, localError);
            if (errMag > maxDef){
              maxDef = errMag;
              maxNode = iNode;
            }
          }
          cout << "max error squared: " << sqrt(maxDef) << "\t" << tol << endl;
        ...
        // notes for starting
        // make datareduction a class variable. Then I can toggle it here to be used as a variable contianing the control nodes for a single specific marker of the inflation layers. 
      ...
      ofstream initial("initcoord" + to_string(rank)+ ".txt");
        ...
        /*--- Loop over inflation layer wall nodes ---*/
          ...
          for ( auto iNode : edge_nodes){
            ...
            GeometryToolbox::Distance(nDim, nodes[iNode]->GetNewCoord(), geometry->nodes->GetCoord(nodes[pointID]->GetIndex()), dist_vec);
        ...
        // revert geometry: 
        // for (auto markers : node_indices[NODETYPE::IL_WALL] ) {
          for( auto iNode : iMarker.second) {
            auto localMarker = nodes[iNode]->GetMarker();
            auto vertex = nodes[iNode]->GetVertex();
            auto var_coord = geometry->vertex[localMarker][vertex]->GetVarCoord();
            for(auto iDim = 0u; iDim < nDim; iDim++){
              /*--- Applying the deformation ---*/
              geometry->nodes->AddCoord(nodes[iNode]->GetIndex(), iDim, -var_coord[iDim]/nIter);
            }
          }
        // }      
        ...
        // insert here the iteration to decide on edge control nodes. 
        ...
        if(dataReductionCfg) {
          ...
          while(sqrt(maxDef) > tol) {
            ...
            for (auto iNode : edge_nodes) {          
              ...
              const su2double* nc = nodes[iNode]->GetNewCoord();
              su2double* c = geometry->nodes->GetCoord(nodes[iNode]->GetIndex());
              const su2double* vc = nodes[iNode]->GetVarCoord();
              su2double delta[3]; 
              for (auto iDim = 0u; iDim<nDim;iDim++) {
                delta[iDim] = nc[iDim] - c[iDim];
                localError[iDim] = -vc[iDim] + delta[iDim];
            ...
            auto tag = config->GetMarker_All_TagBound(marker);
            auto global_marker = config->GetMarker_CfgFile_TagBound(tag);
            ctrl_node_indices[NODETYPE::IL_EDGE][global_marker].insert(maxNode);
          }          
        } else {
          for (auto iNode : edge_nodes) {
            ctrl_node_indices[NODETYPE::IL_EDGE][iMarker.first].insert(iNode);
          }
        }
        ...
        // ofstream layerCoords("layercoords.txt");
        unsigned long nIter_IL = sharpEdge ? 5 : 1;
        nIter_IL = 1;
        ...
        for (auto iStep = 0ul; iStep < nIter; iStep++) {
          ...
          if (DataReduction) { // TODO -  decide on which one.. 
            su2double error[3];
             for (auto x : node_indices[NODETYPE::IL_WALL][iMarker.first]) {
          ...
          ofstream pc_int("post_cor_int.txt");
    ...
    for (auto iNode : edge_nodes) {
      const su2double* varCoord = nodes[iNode]->GetVarCoord();
      for (auto iDim = 0u; iDim < nDim; iDim++) {
        geometry->nodes->AddCoord(nodes[iNode]->GetIndex(), iDim, -varCoord[iDim]/nIter);
      }
    }
  ...
  for (auto marker : node_indices[NODETYPE::IL_WALL]) {
    for (auto x  : marker.second) {
      auto c = geometry->nodes->GetCoord(nodes[x]->GetIndex());
      ver_wall << c[0] << "\t" << c[1] << endl;
    }
  }

  for (auto marker : node_indices[NODETYPE::IL_EDGE]) {
    for (auto x  : marker.second) {
      auto c = geometry->nodes->GetCoord(nodes[x]->GetIndex());
      ver_edge << c[0] << "\t" << c[1] << endl;
    }
  }
...
void CRadialBasisFunctionInterpolation::SolveRBF_System(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const su2double radius, const bool Derivative, const vector<unsigned long>& internalNodes, const bool ForwardProjectionDerivative, const bool Screen_Output){
  ...
  while(((MaxErrorGlobal > dataReductionTolerance)  || greedyIterations == 0)){ 
    ...
    GetInterpolationCoefficients(geometry, config, type, radius, ForwardProjectionDerivative, rhs, invInterpMat);

    for (const auto &iType : slideCtrlTypes) {
      auto it = node_indices.find(iType);
      if (it == node_indices.end() || it->second.empty()) {
        continue;
      }
...
void CRadialBasisFunctionInterpolation::InitializeDataReduction(CGeometry* geometry, CConfig* config, const bool Derivative, unsigned long& maxErrorNodeLocal, su2double& maxErrorLocal ) {
  ...
  maxErrorLocal = sqrt(maxErrorLocal) / ((su2double)nIter);
...
void CRadialBasisFunctionInterpolation::GetInterpolationCoefficients(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const su2double radius, bool ForwardProjectionDerivative, RHS_Data rhs, su2passivematrix& invInterpMat){
...
const bool CRadialBasisFunctionInterpolation::SetBoundNodes(CGeometry* geometry, CConfig* config, const bool Derivative){
    ...
    for (auto iMarker = 0u; iMarker < config->GetnMarker_All(); iMarker++) {
      ...
      for (auto iMarker = 0u; iMarker < config->GetnMarker_All(); iMarker++) {
        ...
        for (auto iVertex = 0ul; iVertex < geometry->nVertex[iMarker]; iVertex++) {
          ...
          else if (hasInflationLayer) {

            // if among primary periodic marker
            if (find(primaryMarker.begin(), primaryMarker.end(), iMarker) != primaryMarker.end()) {
              add_node(nodes, iNode, iMarker, iVertex, NODETYPE::IL_WALL);
            } else if (find(secondaryMarker.begin(), secondaryMarker.end(), iMarker) != secondaryMarker.end()) {
              if (geometry->nodes->GetPeriodicBoundary(iNode)) {
                // include as periodic node
              } else {
                add_node(nodes, iNode, iMarker, iVertex, NODETYPE::IL_WALL);
              }
            } else {
  ...
  for(unsigned long x = 0ul; x < nodes.size(); x++){
    ...
    if(!Derivative && config->GetMarker_All_Deform_Mesh_IL(marker)) {
      if (find(secondaryMarker.begin(), secondaryMarker.end(), marker) != secondaryMarker.end()) {
        marker=sec2prim[marker];
      }
      auto tag = config->GetMarker_All_TagBound(marker);
      ...
      // TODO -  these if else statements can be combined into one
...
void CRadialBasisFunctionInterpolation::GetInterpolationMatrixSequential(CGeometry* geometry, const RADIAL_BASIS& type, const su2double radius, CSymmetricMatrix& interpMat, bool varySupport) const {
  interpMat.Initialize(nCtrlNodesGlobal);

  ofstream phi("phi.txt");
  ...
  for (auto iNode = 0ul; iNode < nCtrlNodesGlobal; iNode++) {
    const auto iOffset = iNode * nDim;    
    su2double dist_te_i = sharpEdge ? GetDistance(CtrlCoords[iOffset], sharpEdgeLoc[0]) : 0.0;

    for (auto jNode = iNode; jNode < nCtrlNodesGlobal; jNode++) {
      const auto jOffset = jNode * nDim;
      su2double dist_te_j = sharpEdge ? GetDistance(CtrlCoords[jOffset], sharpEdgeLoc[0]) : 0.0;
  ...
  //     if (varySupport) {
  //       su2double dist_te1 = GetDistance(CtrlCoords[iOffset], sharpEdgeLoc[0]);
  //       su2double dist_te2 = GetDistance(CtrlCoords[jOffset], sharpEdgeLoc[0]);
  //       su2double dist_te = min(dist_te1, dist_te2);
        
  //       if (dist_te < 1*radius) {
            
  //         su2double reduction_term = (1- dist_te/(1*radius)) * 0.5 * radius;
  //         r -= reduction_term;
  //       }
  //     }

  //     interpMat(iNode, jNode) = SU2_TYPE::GetValue(CRadialBasisFunction::Get_RadialBasisValue(type, r, dist));
  //   }
  // } 
}
...
void CRadialBasisFunctionInterpolation::SetBoundaryDisplacements(CGeometry* geometry, CConfig* config){
  ...
  for (const auto& iType : CtrlTypeVec) {
    ...
    for (const auto& iMarker : markerToNodeSet){
      const auto& markerLocal = config->GetMarker_Local(iMarker.first); // TODO -  this does not return iMarker.first for some reason
      // const auto& markerLocal = iMarker.first;
      ...
      const bool isMoving = ((config->GetMarker_All_Moving(markerLocal) == YES) && (Kind_SU2 == SU2_COMPONENT::SU2_CFD)) ||
        ((config->GetMarker_All_DV(markerLocal) == YES || config->GetMarker_All_Deform_Mesh_Slide(markerLocal) == YES || config->GetMarker_All_Deform_Mesh_IL(markerLocal) == YES) && (Kind_SU2 == SU2_COMPONENT::SU2_DEF)) ||
      ...
      for (const auto iNode : iMarker.second) {
        ...
        if (isMoving) {
          ...
          disp << varCoord[0] << "\t" << varCoord[1] << endl;
        } else  {
            for (auto iDim = 0u; iDim < nDim; iDim++) {
...
void CRadialBasisFunctionInterpolation::GetNodalDeformation(CGeometry* geometry, CConfig* config, const CRadialBasisFunctionNode* const iNode, su2double* varCoord) const {
  ...
  auto nStepsMultiplier = sharpEdge && CtrlTypeVec.size() > 2 ? 5 : 1; // TODO -  
  // nStepsMultiplier = 1;
  
  const su2double VarIncrement = 1.0 / ( (su2double)nIter*nStepsMultiplier);

  const su2double* varCoordCartOrig = iNode->GetNodeType() == NODETYPE::IL_EDGE ? iNode->GetVarCoord() : geometry->vertex[iNode->GetMarker()][iNode->GetVertex()]->GetVarCoord();
...
void CRadialBasisFunctionInterpolation::SetInternalNodes(CGeometry* geometry, CConfig* config, vector<unsigned long>& internalNodes) { 
  ...
  for (auto iMarker : node_indices[NODETYPE::IL_WALL]){
    nWallPoints += iMarker.second.size();
  }
  ...
  for (auto iMarker : node_indices[NODETYPE::IL_WALL]) {
    for (auto iNode : iMarker.second) {
      ...
      PointIDs[nodeCounter] = iNode;
      nodeCounter++;
    }
  }
  // TODO -   move to seperate function 
  CADTPointsOnlyClass adt(nDim, nWallPoints, CoordBound.data(), PointIDs.data(), true);
    ...
    if (!geometry->nodes->GetBoundary(iNode)) {
      ...
      if ( PreserveIL &&  dist < 1.5 * height ) {
        ...
        if (nDistinctTypes >= 2) {
          ...
          // sum all non-IL element counts
          int nNonIL = 0;
          for (const auto &p : vtk_counts) {
              if (p.first != il_type) nNonIL += p.second;
          }

          // avoid divide-by-zero; if no IL elems then can't classify as IL interior
          double ratio = (nIL > 0) ? static_cast<double>(nNonIL) / static_cast<double>(nIL)
                                  : std::numeric_limits<double>::infinity();

          const double ratioThreshold = 1.0; // tune this
          if (nIL > 0 && ratio < ratioThreshold) {
              ...
              if (find(secondaryMarker.begin(), secondaryMarker.end(), nodes[pointID]->GetMarker()) != secondaryMarker.end()){
                ...
                auto tag = config->GetMarker_All_TagBound(marker);
                auto globalMarker = config->GetMarker_CfgFile_TagBound(tag); 

                layerNodes[globalMarker].push_back(iNode);
              }else {
                auto marker = nodes[pointID]->GetMarker();
                auto tag = config->GetMarker_All_TagBound(marker);
                auto globalMarker = config->GetMarker_CfgFile_TagBound(tag);
                layerNodes[globalMarker].push_back(iNode);
              }
              ...
              if (find(secondaryMarker.begin(), secondaryMarker.end(), nodes[pointID]->GetMarker()) != secondaryMarker.end()){
                ...
                auto tag = config->GetMarker_All_TagBound(marker);
                auto globalMarker = config->GetMarker_CfgFile_TagBound(tag);
                node_indices[NODETYPE::IL_EDGE][globalMarker].insert(nodes.size());
                // InflationLayerEdgeNodes[sec2prim[nodes[pointID]->GetMarker()]].push_back();
                nodes.push_back(new CRadialBasisFunctionNode(iNode, sec2prim[nodes[pointID]->GetMarker()], 0));
              } else {
                auto marker = nodes[pointID]->GetMarker();
                auto tag = config->GetMarker_All_TagBound(marker);
                auto globalMarker = config->GetMarker_CfgFile_TagBound(tag);
                node_indices[NODETYPE::IL_EDGE][globalMarker].insert(nodes.size());
                // InflationLayerEdgeNodes[nodes[pointID]->GetMarker()].push_back(nodes.size());
                nodes.push_back(new CRadialBasisFunctionNode(iNode, nodes[pointID]->GetMarker(), 0));
              }
              nodes.back()->SetNodeType(NODETYPE::IL_EDGE);
              nodes.back()->SetILHeight(dist);
              nodes.back()->SetNearestNode(pointID);
          }
          // InflationLayerEdgeNodes[pointID].push_back(nodes.size());
          // nodes.push_back(new CRadialBasisFunctionNode(iNode, pointID, 0 /*// TODO -  does this matter? */ ));
          // nodes.back()->SetNodeType(NODETYPE::IL_EDGE);
          // nodes.back()->SetILHeight(dist);
        }
        ...
        if ( PreserveIL && dist < 1.5 * height ) {
          ...
          if (nDistinctTypes >= 2) {
            ...
            if ( nIL > 0 && ratio < ratioThreshold) {
                ...
                // Mixed or mostly non-IL — treat as IL edge
                if (find(primaryMarker.begin(), primaryMarker.end(), nodes[pointID]->GetMarker()) != primaryMarker.end()) {
                  auto marker = nodes[pointID]->GetMarker();
                  auto tag = config->GetMarker_All_TagBound(marker);
                  auto globalMarker = config->GetMarker_CfgFile_TagBound(tag);
                  node_indices[NODETYPE::IL_EDGE][globalMarker].insert(nodes.size());
  ...
  for (const auto& iType : CtrlTypeVec) {
    ...
    for (const auto& iMarker  : markerToNodeSet) {
      for (const auto iNode : iMarker.second) {    
        ...
        // check from here. I think the wrong normals are used for the inflation layer edge nodes. Since those officially do not have a marker and accompanying vertices....
        auto nn = nodes[iNode]->GetNodeType() == NODETYPE::IL_EDGE ? nodes[iNode]->GetNearestNode() : iNode;
        su2double* norm =  geometry->vertex[nodes[nn]->GetMarker()][nodes[nn]->GetVertex()]->GetNormal();
        su2double mag = GeometryToolbox::Norm(nDim, norm);
        for (auto iDim = 0u; iDim < nDim; iDim++){  
            norm[iDim] = norm[iDim]/mag;
        }
...
void CRadialBasisFunctionInterpolation::SetCorrection_IL(CGeometry* geometry, CConfig* config, const RADIAL_BASIS& type, const vector<unsigned long>& internalNodes, unsigned short marker){
  /*--- The non-selected control nodes still have a nonzero error once the maximum error falls below the data reduction tolerance. 
          This error is applied as correction and interpolated into the volumetric mesh for internal nodes that fall within the correction radius.
          To evaluate whether an internal node falls within the correction radius an AD tree is constructed of the boundary nodes,
          making it possible to determine the distance to the nearest boundary node. 
  ...
  /*--- Number of selected boundary nodes ---*/
  const auto nVertexNonPer = node_indices[NODETYPE::IL_WALL][marker].size() + node_indices[NODETYPE::IL_EDGE][marker].size();
  // const auto nVertexPer   = per_nodes.size();
  auto nVertexBound = nVertexNonPer /*+ nVertexPer*/;
  ...
  /*--- Storing boundary node information ---*/
  unsigned long cnt = 0;
  for (auto iNode : node_indices[NODETYPE::IL_WALL][marker]) {
      PointIDs[cnt] = iNode;
      auto node = nodes[iNode]->GetIndex();
  ...
  for (auto iNode : node_indices[NODETYPE::IL_EDGE][marker]) {
      PointIDs[cnt] = iNode;
      auto node = nodes[iNode]->GetIndex();
      auto coord = geometry->nodes->GetCoord(node);
      for(auto iDim = 0u; iDim < nDim; iDim++){
  ...
  for (auto iNode : node_indices[NODETYPE::IL_WALL][marker]) {
    auto err = nodes[iNode]->GetError();
    for (auto iDim = 0u; iDim < nDim; iDim++) {
      geometry->nodes->AddCoord(nodes[iNode]->GetIndex(), iDim, -err[iDim]);
    }
  }
...
void CRadialBasisFunctionInterpolation::ApplyRBF(CGeometry* geometry, const RADIAL_BASIS& type, const su2double radius, CRadialBasisFunctionNode* const node) {
  // TODO -  add comments
  const su2double* coord = IsCylindrical ? node->GetCylCoord() : geometry->nodes->GetCoord(node->GetIndex()); // TODO -  this was temporarily altered for the inflation layer preservation algorithm
  // const su2double* coord = node->GetNodeType() == NODETYPE::IL_EDGE ? node->GetNewCoord() : geometry->nodes->GetCoord(node->GetIndex());
  ...
  for(auto jNode = 0ul; jNode < nCtrlNodesGlobal; jNode++){
    ...
    // if (varySupport) {
    //   su2double dist_te1 = GetDistance(coord, sharpEdgeLoc[0]);
    //   su2double dist_te2 = GetDistance(CtrlCoords[offset], sharpEdgeLoc[0]);
    //   su2double dist_te = min(dist_te1, dist_te2);
...
// unsigned long CRadialBasisFunctionInterpolation::retrieveIndex(unsigned long index_in) {
//   unsigned long start_idx = 0, end_idx = 0;
  
//   for (const auto& iType : CtrlTypeVec) {
//     const auto& markerToNodeSet = node_indices[iType];
//     for (const auto& iMarker  : markerToNodeSet) {
//       end_idx += iMarker.second.size();
//       if (index_in < end_idx) {
//         auto start = iMarker.second.begin();
//         advance(start, start_idx+index_in);
//         return *start;
//       }
//     }
//   }
...
void CRadialBasisFunctionInterpolation::UpdateVarCoord(CGeometry* geometry, CConfig* config, const CRadialBasisFunctionNode* const node, const su2double* const projection) const {
  ...
  const auto iNonLinear_iter = nIter;
  for (auto iDim = 0u; iDim < nDim; iDim++){
...
void CRadialBasisFunctionInterpolation::CheckSharpEdge(CGeometry* geometry, unsigned long iNode) {

  auto marker = nodes[iNode]->GetMarker();

  auto n1 = geometry->vertex[marker][nodes[iNode]->GetVertex()]->GetNormal();
  auto normal_length = GeometryToolbox::Norm(nDim, n1);
  for(auto iDim = 0u; iDim < nDim; iDim++){
    n1[iDim] = n1[iDim]/normal_length;
  }

  /*--- iterate over edge ---*/
  for (auto i : geometry->nodes->GetEdges(nodes[iNode]->GetIndex())) {
    
    /*--- iterate over nodes of edge ---*/
    for (auto j = 0; j < geometry->edges->GetnNodes(); j++) {
      auto in = geometry->edges->GetNode(i,j);
      
      /*--- If node on boundary and different from considered one ---*/
      if ( geometry->nodes->GetBoundary(in) && in != nodes[iNode]->GetIndex() ){
        
        auto n = geometry->edges->GetNode(i, j);
        auto v = geometry->nodes->GetVertex(n, marker);
        auto n2 = geometry->vertex[marker][v]->GetNormal();
        auto normal_length = GeometryToolbox::Norm(nDim, n2);
        for(auto iDim = 0u; iDim < nDim; iDim++){
          n2[iDim] = n2[iDim]/normal_length;
        }

        su2double dp = GeometryToolbox::DotProduct(nDim, n1, n2);
        if (dp > 0.5) {
          return;
        }
      }
  ...
  sharpEdgeLoc.push_back(geometry->nodes->GetCoord(nodes[iNode]->GetIndex()));
  sharpEdge = true;
}